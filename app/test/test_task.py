...
class TestTask():
    ...
    def test_due_date_error(self):
        with pytest.raises(DueDateError):
            due_date = datetime.now() - timedelta(days=1)
            Task('Title', 'Description', 'BillyBlue', due_date)

    
    @pytest.mark.due_date
    def test_due_date(self):
        due_date = datetime.now() + timedelta(days=1)
        task = Task('Title', 'Description', 'BillyBlue', due_date)
...
# skipif
    ...
    @pytest.mark.skipif(True, reason='La prueba no cumple con los requerimientos')
    def test_skip(self):
        pass